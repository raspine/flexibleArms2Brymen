// TODO: This whole file is a quick hack.
// The knob looks like a I wanted it, however the code rotates/translats things
// back and forth. It could be much simplfied and in addition use working global
// variables such as circle_r/knob_len for customization.
...
module end_stop() {
	difference() {
		union() {
			rounded_extrude(circle_r * 2, round_r) circle(circle_r);
			translate([0, 0, 8]) {
				cylinder(h = 2, r1 = circle_r + round_r, r2 = circle_r + round_r);
			}
		}
		translate([0, 0, 4]) rotate([90, 0, 90]) {
			linear_extrude(2*circle_r) {
...
module knob() {
	translate([0, 0, 75]) rotate([0, 180, 0]) {
		translate([0, 0, 40]) {
			translate([0, 0, -21]) start_stop();
			cylinder(h=25, r1=20, r2=20, center=false);
		}
